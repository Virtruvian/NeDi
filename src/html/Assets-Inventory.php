...
<td>
	...
	<input type="text" title="<?= $invlbl ?> <?= $typlbl ?>" placeholder="<?= $typlbl ?>" name="ty" value="<?= $ty ?>" class="m" OnFocus="select();" onClick="window.open('inc/browse-img.php?t=p','Panels','scrollbars=1,menubar=0,resizable=1,width=600,height=800');" title="<?= $sellbl ?> <?= $imglbl ?>">
	<input type="text" title="<?= $invlbl ?> <?= $numlbl ?>" placeholder="<?= $numlbl ?>" name="as" value="<?= $as ?>" class="m" OnFocus="select();">
	<input type="text" title="<?= $invlbl ?> <?= $loclbl ?>" placeholder="<?= $loclbl ?>" name="lo" value="<?= $lo ?>" class="l" OnFocus="select();">
	<br>
...
if ($add and $sn and $ty){
	$query	= GenQuery('inventory','i','','','',array('serial','type','usrname','asupdate','assetlocation','state','comment','endwarranty','pursource','assetnumber','purcost','purnumber','purtime','maintpartner','startmaint','endmaint'),array(),array($sn,$ty,$_SESSION['user'],time(),$lo,$sta,$com,($wint === FALSE)?0:$wint,$ps,$as,($pc)?$pc:0,$pn,($tis === FALSE)?0:$tis,$mp,($sms === FALSE)?0:$sms,($ems === FALSE)?0:$ems) );
	if( !DbQuery($query,$link) ){echo "<h4>".DbError($link)."</h4>";}else{echo "<h5>$serlbl $sn $updlbl OK</h5>";}
}elseif ($upd and $sn and $ty and $lo){
	$query	= GenQuery('inventory','u',"serial = '".DbEscapeString($sn)."'",'','',array('assettype','usrname','assetupdate','assetlocation','state','comment','endwarranty','pursource','assetnumber','purcost','purnumber','purtime','maintpartner','startmaint','endmaint'),array(),array($ty,$_SESSION['user'],time(),$lo,$sta,$com,($wint === FALSE)?0:$wint,$ps,$as,($pc)?$pc:0,$pn,($tis === FALSE)?0:$tis,$mp,($sms === FALSE)?0:$sms,($ems === FALSE)?0:$ems) );
	if( !DbQuery($query,$link) ){echo "<h4>".DbError($link)."</h4>";}else{echo "<h5>$serlbl $sn $updlbl OK</h5>";}
}elseif($del ){
...
<?php
$query	= GenQuery('inventory','g','assettype');
$res	= DbQuery($query,$link);
if($res){
	$row = 0;
	while( ($item = DbFetchRow($res)) ){
